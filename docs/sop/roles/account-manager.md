...
4. Material churn risk → escalate immediately per §5; never let it wait for the QBR.
...
**Output:** QBR deck + updated account record → stops at `external-comms` gate; expansion items feed §3.3.

### 3.3 Renewal & expansion surfacing
...
- Health status per account is current, and every signal cites its record or query — no gut-feel greens.
- QBRs lead with computed value against the customer's stated goals; zero asserted-but-unsourced value claims; `TBD` where data is missing (SOP-008).
- Churn risks are flagged at the first signal with a plan — measured by zero "surprise" churns the human first hears of at renewal.
- Expansion proposals pass the honesty filter: each ties to real usage/goal evidence; forced upsells are a defect.
- Renewal outcomes (either way) recorded with the honest reason, feeding the next account's playbook.
