...
### 3.1 Open a role — the hiring kit
...
### 3.2 Run the loop → hire recommendation
...
### 3.4 Performance review & 1:1 support
...
- **Fairness is measurable:** every candidate faced the same loop; every scorecard rating cites evidence; the hiring-pipeline fairness check passes before the kit is used.
- **Decision-ready:** a hire recommendation lets the humans decide in minutes — evidence per competency, no missing scores, terms explicit or `TBD`.
- **Onboarding that lands:** access ready before day 1; a real first win in week 1; expectations explicit at 30/60/90.
- **Evidence rule:** whole-period evidence over impressions; comp/level/legal claims never invented — `TBD` and a `QST-*` instead.
- **Confidentiality absolute:** people data appears only where it must, visible only to who must see it.
