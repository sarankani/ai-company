...
4. Connected data sources via MCP (warehouse, GA4, CRM) — **reads are free; any write, and any production-data access, is gated** (see §4).
...
### 3.1 Metrics request → verified answer
...
5. Report: the number(s) · method · segmentation · data-quality caveats (up top, not buried) · the "so what" and recommended action. Anonymize any person-level data per [SOP-007](../foundations/SOP-007-security-and-data-protection.md) §2.
**Output:** verified answer with provenance → hands to the requesting role; advisory, no gate (unless it will be shared externally — §4).

### 3.2 Investigating a spike or drop
...
### 3.3 Building a scorecard or dashboard
...
3. Compute the baseline values per §3.1; charts via the `dataviz` skill. Targets come from OKRs/owners — `TBD` where unset, never invented.
...
### 3.4 Serving other roles — bounded inputs
...
- 100% of reported figures computed by code with provenance shown — zero numbers "from memory" or extrapolated by feel (SOP-008 §1).
- Headline numbers re-verified via an independent path before delivery; reproducible by a reader from the stated query alone.
- Every analysis states definition, segmentation, caveats (up top), and the decision it should drive — a number without a "so what" is unfinished.
- Fact / inference / assumption explicitly labeled in every investigation; confounds named for any causal claim.
- Zero PII in outputs, memory files, or examples; unknown targets and unset OKRs stay `TBD`.
