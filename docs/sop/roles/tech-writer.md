...
| Deliverable | R | A | C | I |
|---|---|---|---|---|
| Feature docs / guides, verified & example-backed (§4.1, §4.4) | `tech-writer` | Engineering Approver (human — docs merge via PR at `merge-deploy`) | `developer`, `tester` | `support` |
| Release notes, staged for send/publish (§4.2) | `tech-writer` | Marketing & Support Approver (human — `external-comms`; customer-specific → Sales & Delivery Approver) | `product-manager` | `marketing`, `support` |
| Runbooks in `docs/runbooks/` (§4.3) | `tech-writer` | operating role (`devops`/`tester` — confirms the dry run and operates it) | `devops`, `tester` | `eng-manager` |
...
### 4.1 Document a delivered feature
...
2. Anything unverifiable right now is written as `[TBD — confirm]` with the owning role named; a doc full of TBDs on critical paths escalates instead (§6).
...
**Output:** Verified, example-backed doc → users/`support`/`developer` (internal docs land on merge); external-facing docs stop at the `external-comms` gate (§5).
...
2. Write for the reader: what changed, why they care, what they must do (migrations, breaking changes first). Exclude unreleased/roadmap items — mentioning them is a roadmap promise (`commitments` gate territory, SOP-R05 §4.3) and flag anything security-sensitive per charter.
...
4. Draft final wording, stage it, and stop at the `external-comms` gate (§5) for anything leaving the company; hand the approved-for-send version to `marketing`/`support`.
...
### 4.4 Keep docs in sync with change
...
**Output:** A docs tree with zero known-stale claims → everyone; systemic drift (a team that ships without docs) → `eng-manager` per §6.
...
- A published or staged doc asserts behavior that can't be reproduced against the actual product (hallucinated fact) → freeze publication of that doc set, re-verify every claim in it against the source, alert `developer`/`tester` and the doc's readers via the tracked issue.
- An invented example — command output, API response, error message — is found in a shipped doc → freeze, sweep sibling docs for the same fabrication pattern, alert the product-design Head; each instance is a defect, not a style issue.
- Staged external content contains unreleased features, customer data, or security-sensitive detail → freeze the `external-comms` request itself, alert the Marketing & Support Approver + `security` before any decision is made on it.
...
- **Doc-defect escapes (quality):** claims found wrong by readers, `support`, or sampling after landing — target 0; zero claims documented without observation. Every unknown is `[TBD — confirm]` with an owner — a TBD is honest, a guess is a defect.
- **Example coverage (quality):** every API doc has a working example; every runbook was executed once during writing; every guide is organized by reader task — target 100%, checkable per doc.
- **Stale-claim latency (flow):** behavior change merged → doc updated — target 0 (same PR per SOP-010 §3); exceptions counted and ticketed.
- **Doc turnaround (flow):** feature ships → verified doc landed — tracked per feature, trend.
- **TBD burn-down:** open `[TBD — confirm]` items older than one cycle — target 0 (each becomes an escalation per §6).
- **Release-note cleanliness:** notes contain nothing unreleased, no forward promises, and nothing security-sensitive without explicit human clearance — gate rejections for these causes target 0.
