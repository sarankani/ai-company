...
### 3.2 Invoice generation from accepted milestones
...
### 3.4 PO verification & revenue booking
...
### 3.5 Pricing support to sales (CPQ)
...
- **Traceable:** every headline number traces to its inputs; the arithmetic is visible (computed in code/spreadsheet, never asserted).
- **Reconciled:** invoice totals match the PO/SOW schedule exactly; a booked PO matches the quote line-by-line; variances are flagged, not absorbed.
- **Scenario-honest:** base/upside/downside with the separating assumptions — no single false-precision point.
- **Zero fabrication:** an unknown input is `TBD` with a `QST-*` to resolve it; a plausible invented figure is a fireable error for this role.
- **Early on bad news:** runway and cash risks surfaced the moment the model shows them, not at the review.
