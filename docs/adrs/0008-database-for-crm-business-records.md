...
- The 16 business-record entity types live in a Postgres database (Neon in production, any Postgres in Docker/dev, PGlite in tests), accessed exclusively through the panel's CRM store (`panel/lib/crm/`) — one write path that enforces lifecycle validation, id allocation, and append-only activity history.
...
- The panel is no longer fully stateless: it owns a database. Backup/restore of the CRM data becomes an operational responsibility (Neon PITR in production; documented in the runbook).
