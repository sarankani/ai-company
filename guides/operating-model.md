...
|---|---|---|
| `ceo` | Strategy, priorities, OKRs, board/investor comms | `/okrs`, `/board-update`, `/strategy-review` |
| `eng-manager` | Engineering delivery, unblocking, 1:1s, team health | `/one-on-one`, `/team-report`, sprint-plan |
| `hr` | Hiring, onboarding, performance, culture, policy | `/job-description`, `/hiring-plan`, `/onboarding-plan`, `/performance-review` |
| `finance` | Budget, runway, forecasting, invoicing | `/budget-plan`, `/financial-model` |
| `product-manager` | What to build & why: specs, roadmap, prioritization | `/spec`, `/roadmap`, `/metrics-review` |
| `project-manager` | Delivery: sprints, standups, tracking, risk | `/sprint-plan`, `/standup` |
| `designer` | UX/UI, flows, design system, prototypes | `/design-brief`, design-critique, handoff |
| `tech-writer` | Docs, guides, API docs, release notes | `/documentation`, release notes |
| `developer` | Implement features, fix bugs, write tests, open PRs | code + `/refactor-plan`, test-writer |
| `code-reviewer` | Review changes for correctness/security/perf | `code-review`, adversarial-verifier |
| `tester` | Test strategy, generation, regression, bug triage | `/test-strategy`, test-gap, `/ticket-triage` |
| `devops` | CI/CD, deploys, infra, incidents | deploy-readiness, `/incident`, `/postmortem` |
| `security` | AppSec review, threat modeling, vuln triage | code-review (security lens), ml-security-audit |
| `sales` | Outreach, discovery, proposals, pipeline | `/sales-outreach`, `/proposal` |
| `marketing` | Campaigns, content, SEO, launches | `/campaign-brief`, `/content-calendar`, `/seo-brief` |
| `social-media` | Social calendar, posts, engagement | `/social-post`, `/content-calendar` |
| `support` | Ticket triage, repro, customer responses | `/ticket-triage`, `/support-macro` |
| `data-analyst` | Metrics, dashboards, insight | `/metrics-review`, dataviz |
