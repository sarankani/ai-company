...
## 2026-07-19 — Roster 24 → 27; Roles & Designations framework (Plan 006)

**Who:** Saran (decided: map the full ~120-designation org chart onto the archetypes AND close the structural gaps — not one-agent-per-title) / chief-of-staff+hr+tech-writer lens (executed). Added 3 archetype agents for functions that mapped to nobody: `ml-engineer` (Data & AI build → engineering), `it-admin` (IT & Admin → operations), `legal-counsel` (Legal & Compliance → operations; prepares only, binding advice/signature stay human/external counsel). New canonical record `company/org/designations.md` maps every real-world title → its operating agent, making explicit that one agent = a role family (VP→IC ladder + specializations) and that C-suite authority stays with human seats (prepares vs decides). **No new gate authority** — legal signature routes via existing `money`/`commitments` + CEO; a dedicated `legal` gate is a possible future ADR. `departments.md` org change (Head-proposes/CEO-approves) carried by the plan's approval. Full sweep done same change (Founder: "update all documents and diagrams"): 3 role SOPs (R25/R26/R27, full template + §7.1 diagrams), all 4 `docs/org/agent-map-*` diagrams, CLAUDE.md/README/guides/memory count 24→27, marketing snapshots refreshed (AGI posts flagged for human re-confirm pre-publish). Resolves the long-standing README/count drift. Branch: `claude/org-roles-designations`; PR #105; Plan `docs/plans/006-roles-and-designations.md`.

## 2026-07-19 — CRM merged to main (PR #88): Epic E7 delivered

**Who:** Saran (merged) / developer (executed). The full CRM — 16 entity types in Postgres (ADR-0008 model), role × department RBAC, /crm screens + pipeline + notifications, agent API/CLI, git-native gate bridge — is on main. CI green (unit/e2e/a11y/audit/secret-scan). Issues #80-85/#87 closed. Open human gates: flip ADR-0008 to Accepted (#79) and provision Supabase + authorize the panel deploy (#86); until the prod `DATABASE_URL` is set, the deployed panel's CRM runs on ephemeral local storage.

## 2026-07-16 — Company CRM built into the panel (Epic E7, EX-701…709) — in review

**Who:** Saran (decided scope + stack) / developer+solutions-architect (executed). Decisions: full-lifecycle CRM (all 16 entity types) in the panel; member RBAC as a **role × department matrix** (`crm-viewer`/`crm-editor` grants in humans files; chain seats implicit editor; no grant = hidden; grants never decide); **Postgres for business records** — ADR-0008 (Proposed) partially supersedes ADR-0002, git keeps approvals/questions/org; **Supabase** chosen as prod Postgres + Realtime notifications; Inter as panel typeface. Built on branch `claude/company-crm-control-panel-mfa1wz`: schema+store+RBAC, /crm screens + opportunity kanban + notifications bell, agent API + `scripts/crm.mjs`, gate bridge (gated stage moves file APRs, exactly-once apply — verified live incl. a Python-engine decision), gated add-member flow in /admin. **Security fix landed both sides:** `authorized()` (TS + Python) now requires a chain/ceo seat — any-seat-in-department would have let members decide. Verified: 179 Vitest + 31 Playwright + 32 pytest green, prod build green. Awaiting: ADR-0008 acceptance (#79), merge, Supabase provisioning + deploy authorization (#86). Plan: `docs/plans/005-crm-module.md`; epic #78.

## 2026-07-15 — /visualize-agents skill created; visual role-flow diagrams rolled out across all SOPs

**Who:** Saran (requested) / tech-writer lens (executed, 7 parallel department batches). New skill `.claude/commands/visualize-agents.md` (org/gates/value-chain/workflows/agent-detail views, Mermaid, grounded in `company/org/` + `.claude/workflows/` — never from memory). Rolled out: §7.1 "Role flow — visual" in all 24 role SOPs (v1.2) + §1b gate landscape in human-seats (SOP-R00) + 4 company-wide maps in `docs/org/agent-map-*.md` + convention added to `_template.md` and the SOP README. Branch: claude/agent-visualization-skill-pte6oq. **Drift backlog surfaced while diagramming (needs owner decisions, not yet fixed):** (1) `money`-gate ownership contradiction — routing.md says people-finance, but sales/account-manager SOP RACIs claim the Sales & Delivery seat decides discounts/concessions; (2) charters staler than SOPs — eng-manager/devops wording implies they "hold" the merge-deploy gate, eng-manager pod omits code-reviewer, sales charter hands won deals to support/project-manager instead of delivery-manager, delivery-manager/account-manager hardcode `ceo` escalation; (3) project-manager/designer charters lack the Bash grant their own gate protocol requires (`approval_engine.py`); (4) several SOPs never mention the workflows that invoke them (company-standup/product-launch/delivery-to-invoice); (5) count drift — README says 23 employees (actual 24), getting-started says 16 skills (actual 28); (6) tester↔devops release-candidate trigger circularity.
...
## 2026-07-15 — SOP library created: docs/sop/ (foundations + all 24 roles)

**Who:** Saran (requested) / tech-writer lens (executed). World-class SOP library written to be loaded by AI agents: 11 foundation SOPs (SOP-000…010 — standards/change control, session protocol, system of record, approval gates, escalation/SLAs, task lifecycle, handoffs, security, quality/evidence/honesty, incidents, docs & memory), a mandatory role-SOP template, and one role SOP per the 23 AI employees plus SOP-R00 for the four human seats (Approver/Deputy/Head/CEO). Design: universal best practice first, Evalyn binding second; precedence ADRs > CLAUDE.md > foundations > role SOPs > guides. Change control: foundations/gate-touching edits need Founder/CEO stamp; role SOPs need the owning department's Approver; gates never weakened via SOP edit (needs superseding ADR). Grounding note: several agent charters name skills with no local command file (/spec, /sprint-plan, /standup, /seo-brief, /team-report, /strategy-review, /metrics-review, /kickoff) — the SOPs cite only real ones and mark those TBD. Branch: claude/ai-company-sop-docs-84ojgq.
